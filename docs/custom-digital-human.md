...
若 **`head` 网格或 `extra_meshes` 中任一对象**带有与 SRanipal 列名一致的 **`Tongue_*` 形态键**（见 `face_blender_shape.core.blendshape_schema.BLENDSHAPE_NAMES`），启动时会自动启用 **舌头直连**：这些通道按 CSV 数值写入对应形态键，且 **SRanipal 中全部舌头行不再参与 ARKit 映射**（避免 `Tongue_LongStep*` 再叠加到 `jawOpen`）。无 `Tongue_*` 键时行为与此前一致。
...
- **`head`**：可选；覆盖 FBX 里作为主脸的网格对象名（默认 MetaHuman 头对象名见 `face_blender_shape.core.asset_names`）。
